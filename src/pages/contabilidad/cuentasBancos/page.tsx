...
const CuentasBancosPage = () => {
    ...
    const { contextCuentasBancos: { state: { procesando }, nuevo } } = useData()
    const [filter, setFilter] = useState<string>('')
    const { ButtonPrimary, Searcher, TitlePage } = useComponents()
    const { IconPlus, IconBankAccount } = useIconos()
    ...
    return (
        <>
            <Container
                title={
                    <>
                        <TitlePage title="Cuentas de Bancos" icon={<IconBankAccount style={{ fontSize: 24 }} />} />
                        <Space>
