...
const FacturasListado = (props: Pick<ControlProps, "filter" | "onClick">) => {

    const { contextFacturas: { state: { procesando, recargar }, abiertas } } = useData()
    ...
    const cargar = async () => {
        ...
        const resp = await abiertas();
        if (resp && resp.ok && resp.datos) {
            setFacturas(resp.datos)
        }
    }
        ...
        <Table<Factura>
            ...
            <Column title="Acci&oacute;n" align="center" fixed="right" width={80} render={(record: Factura) => (
                <Tooltip title={`Editar la factura número (${record.numero})`}>
                    <Button type="text" icon={<EditFilled className="fs-5" />} onClick={() => onClick && onClick(record)} />
                </Tooltip>
            )} />
        </Table>
    )
